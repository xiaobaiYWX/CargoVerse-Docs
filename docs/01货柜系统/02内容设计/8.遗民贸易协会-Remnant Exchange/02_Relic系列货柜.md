...
| `luxury` | 贵重消费品 | √ | SP | `keep_upright`<br>`fragile` | 定制防护结构适合保存前文明遗物，可运输珠宝、艺术品和高价值消费品。 |
| `research_sample` | 科研样本 | √ | SP | `time_limit`<br>`fragile`<br>`keep_upright`<br>`light_sensitive` | 定制防护结构适合保存前文明遗物，可运输生物、地质和遗迹研究样本。 |
| `data` | 数据资源 | √ | SP | `cold_proof`<br>`fragile`<br>`moisture_proof` | 定制防护结构适合保存前文明遗物，可运输数据服务器、存储阵列和加密资料。 |
| `experimental` | 实验品 | √ | SP | 无 | 定制防护结构适合保存前文明遗物，可运输实验原型、未知装置和测试设备。 |

---

## REX-Relic-【INTERFACE】-SP

### REX Relic 遗迹保存舱 SP 型

> 围绕来源不明、尺寸异常或极易损坏的前文明遗物临时定制，拼接式防护结构和晦涩封条通常只有协会鉴定人能够解释。该 SP 型不按常规体积划分，将依据具体货物、设备或任务单独配置。
...
| 参数 | 名称 | 内容 |
| --- | --- | --- |
| `template` | 货柜结构ID | `cargoverse:rex/relic/rex_relic_【interface】_sp` |
| `display_name` | 显示名称 | `REX Relic 遗迹保存舱 SP 型` |
| `cargo_description` | 货柜描述 | `§lREX Relic 遗迹保存舱 SP 型\n§r§7围绕来源不明、尺寸异常或极易损坏的前文明遗物临时定制，拼接式防护结构和晦涩封条通常只有协会鉴定人能够解释。该 SP 型不按常规体积划分，将依据具体货物、设备或任务单独配置。\n§r§l制造商：Remnant Exchange\n§r§l系列：Relic Series\n§r§l接口：【待确定】` |
