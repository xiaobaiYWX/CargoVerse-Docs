...
| 产品系列 | 规格 | 接口 | 定位 | 特点 |
| --- | --- | --- | --- | --- |
| Fortress 系列 | XL/SP | 【待确定】 | 堡垒部署模块 | 前线基地；大型装备 |
...
| `textile` | 纺织与轻工业品 | × | — | 无 |  |
| `security` | 安防与武装设备 | √ | XL/SP | 无 | 堡垒化结构可整体部署大型防务设施，可运输武器、装甲、雷达和安防设施。 |
| `component` | 工业零件 | × | — | `fragile` |  |
| `machinery` | 机械设备 | √ | XL/SP | `keep_upright`<br>`fragile` | 堡垒化结构可整体部署大型防务设施，可运输机床、动力设备和工程机械。 |
| `electronics` | 电子设备 | × | — | `moisture_proof`<br>`fragile` |  |
| `chemical` | 化工材料 | × | — | `radioactive` |  |
| `construction` | 建筑材料 | √ | XL/SP | 无 | 堡垒化结构可整体部署大型防务设施，可运输建材、管线和设施配套材料。 |
| `structural_module` | 大型结构组件 | √ | XL/SP | 无 | 堡垒化结构可整体部署大型防务设施，可运输预制舱段、设备模块和大型结构件。 |
| `supplies` | 生活补给 | √ | XL/SP | 无 | 堡垒化结构可整体部署大型防务设施，可运输食品、工具和聚落日常补给。 |
...
| 参数 | 名称 | 内容 |
| --- | --- | --- |
| `template` | 货柜结构ID | `cargoverse:bds/fortress/bds_fortress_【interface】_xl` |
...
| 参数 | 名称 | 内容 |
| --- | --- | --- |
| `template` | 货柜结构ID | `cargoverse:bds/fortress/bds_fortress_【interface】_sp` |
