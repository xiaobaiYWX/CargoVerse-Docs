...
> 已同步内置数据包；2026-09-14 按平面先行流程完全重构六个 NBT，平面与概念图见下文。游戏内验收与拼接待作者完成，自然生成尚未接入。
...
## 已交付独立建筑模板（2026-09-14 平面驱动重构版）

本版从平面重新制作六个模板，替换原有同名 NBT。实际业务区统一为水平地面 Y=0，北侧为入口／接近方向；异常错位外观集中在独立观测架。只使用 Java 1.21.1 原版方块，业务终端保留空间，不附带实体、库存或实例绑定。

存档目录：`dev_world/generated/cargoverse/structures/trade_points/resource/anomaly/far_reach_exchange/`。仅交付 `.nbt`，原有六个加载名称继续使用。

### 平面先行

先安排接待、值守、保管、交接与停靠区域，再从边界和跨度确定柱位；依次搭建柱梁与屋顶骨架、墙窗及屋面、最后补檐口、柱脚、窗台、栏杆和灯具。平面图、建造代码和通道检查共用同一份分区与柱位数据。
...
图片由本次 NBT 方块与原版模型几何离线绘制，颜色采用材质平均色，灯光与部分特殊方块外形有简化；非游戏截图。
...
接收棚承担平台后的分类与人工交接，飞行载具使用独立航空平台。观测架属于外围景观，放在飞行路线之外；后部展位不作为人员穿行通道。模板之间的拼接与场地处理由作者完成。

### 本次针对性检查

生成时检查原版方块名称／状态与坐标范围，并针对这次结构重构检查声明通道的地面、两格通行高度和预留体积冲突。门按可开启通道处理。这些检查仅覆盖已声明区域，不代表完整游戏物理或载具适配验收；未操作游戏、未进行整套反复校验。
