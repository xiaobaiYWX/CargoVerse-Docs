...
模板统一采用浅木、蓝绿色与白色框架。池塘由模板自带，航空装货和卸货采用两个独立露天平台。仅使用 Java 1.21.1 原版方块，不附带动物实体、模组业务终端或实例绑定。
...
```text
<存档目录>/generated/cargoverse/structures/trade_points/resource/ecology/aquaculture_station
```
...
![云泽水产与育种站六个建筑概念总览](../../../../assets/images/trade_points/resource/ecology/aquaculture_station/overview.png)
...
| 禽羊蜂群与鱼苗育种温室 | [查看外观](../../../../assets/images/trade_points/resource/ecology/aquaculture_station/breeding_conservatory_exterior.png) | 四区由十字通道分开；禽羊围栏、空蜂箱、花圃及鱼苗水池仅提供空间，动物需另行添加。 |
...
航空平台没有中央顶棚或横梁；其 NBT 高度只包含已建构件，不会自动清除更高处已有的地形或障碍。实际起降空间需按载具尺寸与周边地形调整；放置后检查场地衔接、池水边界及生物活动空间。
