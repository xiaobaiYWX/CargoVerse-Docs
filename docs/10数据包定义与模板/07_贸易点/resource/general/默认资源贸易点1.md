...
包含 **3 个独立 NBT**，使用 Minecraft Java 1.21.1 原版方块。业务设备预留安装空间，模板不附带实例绑定或自然生成配置。

以下为默认测试点的可选附属模板，原有云天农庄结构与功能定义独立保留。

存放路径：`<存档目录>/generated/cargoverse/structures/trade_points/resource/general/default/`。模板文件格式为 `.nbt`。
