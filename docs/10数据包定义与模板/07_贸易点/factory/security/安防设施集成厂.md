...
工业航道防护区；出售安防系统、巡逻载具与预制工事。

| 项目 | 内容 |
| --- | --- |
| 设计编号 | B02 |
| 资源 ID | `cargoverse:factory/security/security_works` |
| 目标 JSON | `src/main/resources/data/cargoverse/trade_points/trade_points_definitions/factory/security/security_works.json` |
| schema_version | 1 |
| manufacturer | `cargoverse:bds` — 铁壁防务集团 / Bastion Defense Systems |
| display_name | 安防设施集成厂 |
| node_role | `security_supplier`（语义元数据） |
| tech_level | 3（目录中最高货物科技等级的描述，不是解锁条件） |
| 布局阶段 | 工业与专业航线（地图投放建议，不是运行时字段） |
| ticks_per_day | 24000 |
| resource_table | [cargoverse:factory/security/security_works](<../../../06_资源表/factory/security/security_works.md>) |

## 收售概览

出售：周界警戒系统、预制防御工事、轻型巡逻艇、污染吸附剂。

收购：工业蓄能电池、通信中继器、光学玻璃坯。

货物标签、逐项初始库存、容量和日变化统一见关联资源表；两侧各自维护库存。

## 建议结构内容

**建筑形象：**灰色装甲平台、蓝白设备仓与黄色巡逻泊位线；外部设备分区明显。

| 建筑／分区 | 建议内容 |
| --- | --- |
| 安防系统展示墙 | 展示周界警戒设备、通信模块和传感设备的安装形式。 |
| 预制工事装配场 | 陈列一段可辨认的防御工事样件，不占据实际货柜装卸位。 |
| 巡逻艇检修泊位 | 用静态巡逻艇轮廓或空检修架表达载具供应。 |
| 污染材料返运间 | 污染吸附剂设封闭外运仓，和电子仪器来料保持分隔。 |

**行走与装卸路线：**营业前台靠近普通泊位；成品装货与电子／能源来料卸货分置两侧。
...
| 项目 | 设计值 |
| --- | --- |
| 世界结构 ID | `cargoverse:trade_point/factory/security/security_works` |
...
| 绑定文件 ID | `cargoverse:factory/security/security_works` |
| 绑定候选 | 仅 `cargoverse:factory/security/security_works`，权重 1，概率 100% |
