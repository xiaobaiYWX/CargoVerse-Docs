...
生态与医疗设施区；供应药品、培养材料和生态模块。

| 项目 | 内容 |
| --- | --- |
| 设计编号 | V03 |
| 资源 ID | `cargoverse:factory/biology/biosystems_station` |
| 目标 JSON | `src/main/resources/data/cargoverse/trade_points/trade_points_definitions/factory/biology/biosystems_station.json` |
| schema_version | 1 |
| manufacturer | `cargoverse:vbs` — 绿洲农业联盟 / Verdant BioSystems |
| display_name | 绿洲生物工程站 |
| node_role | `biotech_supplier`（语义元数据） |
| tech_level | 4（目录中最高货物科技等级的描述，不是解锁条件） |
| 布局阶段 | 工业与专业航线（地图投放建议，不是运行时字段） |
| ticks_per_day | 24000 |
| resource_table | [cargoverse:factory/biology/biosystems_station](<../../../06_资源表/factory/biology/biosystems_station.md>) |

## 收售概览

出售：急救药剂、再生培养液、无菌敷料、疫苗制剂、循环水培舱、培养基原液、水培营养液。

收购：纤维棉包、养殖鱼苗、高空微生物样本。

货物标签、逐项初始库存、容量和日变化统一见关联资源表；两侧各自维护库存。

## 建议结构内容

**建筑形象：**白色封闭舱体、绿色植物窗与干净的玻璃连廊；人工生态感强于普通农场。

| 建筑／分区 | 建议内容 |
| --- | --- |
| 医疗制剂仓 | 急救药剂、敷料、疫苗和培养液分别用标识清楚的封闭货架陈列。 |
| 培养与检测室 | 透明培养罐、样本台和观察窗表现生物研究。 |
| 生态模块展台 | 展示一段循环水培舱与营养液管线，模块主体不侵入货柜泊位。 |
| 样本与种群接收舱 | 鱼苗箱、棉包和微生物样本分开摆放，形成清楚的收货背景。 |

**行走与装卸路线：**访客营业入口与工作舱分离；药品装货区遮蔽，活体和原料卸货口位于另一侧。
...
**可选扩展：**可选添加观景植物舱、员工休息舱和备用试验室；外观洁净不增加无菌判定系统。
...
| activity.expected_daily_trade_value | 8100 |
...
| 项目 | 设计值 |
| --- | --- |
| 世界结构 ID | `cargoverse:trade_point/factory/biology/biosystems_station` |
...
| 绑定文件 ID | `cargoverse:factory/biology/biosystems_station` |
| 绑定候选 | 仅 `cargoverse:factory/biology/biosystems_station`，权重 1，概率 100% |
