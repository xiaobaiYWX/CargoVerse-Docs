...
> 五类 Farmstead 建筑模板的空间布局、柱网、原版构件与朝向差异分析。

## 来源与分析口径

参考模板位于整合包游戏目录下的 `kubejs/data/farmstead/structures/`。

- 20 个模板的 DataVersion 均为 3465；按文件名分为 `barn`、`hangar`、`kitchen`、`merchant_camp`、`tool_shed` 五组，每组东南西北四份。
...
保存边界未必包含建筑下方的完整地面，不能据此认定缺少支撑。后续引用时需要明确接地层，保留地形衔接空间；对独立航空平台才按作业需要制作完整甲板。
...
北向样本中原版方块位置占约 89%～98%，主体结构以原版构件为主。模组内容主要是任务屏幕、箱柜、炊具、货物、机械部件等。

但部分内容也参与局部外观，例如机库有 40 个 `create:white_sail` 和 16 个 `create:fluid_tank`。主体分析图中隐藏了这些模组构件；以后做纯原版版本时，可用原版帆布、储罐造型替代，不应把“隐藏后的缺口”作为目标设计。
...
以上为可复用组件的设计建议。组件用于实现已经确定的平面和柱网，可在制作过程中逐步积累；引用时保留可靠的连接关系，再根据目标空间适配。

## 平面先行的建造流程

这五类建筑采用先安排平面、再根据平面确定柱间距的建造方法。平面中的用途、通道和高差决定后续骨架与围护结构。

主体模板范围包括大体空间布局、柱网框架、楼地面、必要隔墙、墙面门窗、屋顶和楼梯。细节装饰与室内设计属于后续装修阶段；参考样本中的家具、灯具及细部方块数量不作为主体模板要求。
...
3. **围护与通行。** 在框架之间布置墙、门、窗及屋面，完成楼梯和上下层连接，使围护与平面中的使用和通行需求一致。
4. **后续装修。** 主体完成后再安排室内布置、灯具和装饰；半砖、倒置楼梯和活板门可用于细部塑形。此阶段不包含在默认主体 NBT 的制作范围内。

建造时使用一致的平面、柱位和高度数据，按上述顺序推进。构件提取、方块状态处理及分析图用于辅助空间和构件设计。

分析图片归档于文档库内的 `assets/images/trade_points/reference/farmstead/`，使用标准 Markdown 相对链接。
